...
h_rec = [h_init]
b_rec = [b_ML_init]
hb_rec = [MLMML.OC_getIntegratedBuoyancy(occ, 1, 1)]
we_rec = []
Δb_rec = [occ.b_ML[1, 1] - occ.bs[1, 1, occ.FLDO[1, 1]]]
bs_rec = zeros(Float64, occ.Nz, length(t))
...
for k = 1:length(t)-1
    ...
    push!(h_rec, occ.h_ML[1, 1])
    push!(b_rec, occ.b_ML[1, 1])
    #push!(Δb_rec, info[:Δb])
    push!(hb_rec, MLMML.OC_getIntegratedBuoyancy(occ, 1, 1))
    #push!(we_rec, ( info[:flag] == :we ) ? info[:val] : NaN)
    bs_rec[:, k+1] = occ.bs
end


for i=1:length(occ.bs)
...
for i = 1:5:size(bs_rec)[2]
    plt[:plot](bs_rec[:, i], (occ.zs[1:end-1] + occ.zs[2:end]) / 2.0, "k-")
end
