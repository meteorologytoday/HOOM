...
function init(ocn::Ocean)
    ...
    @sync for (i, p) in enumerate(workers())
        ...
        @spawnat p let
            global subocn_vt = makeSubOcean(o, i, beg_y_idxs[i], sub_Nys[i], :vt)
            global subocn_hz = makeSubOcean(o, i, beg_z_idxs[i], sub_Nzs[i], :hz)
        end
...
function run!(
    ...
    (o.id == 0) || throw(ErrorException("`id` is not 0 (master). Id received: " * string(o.id)))
