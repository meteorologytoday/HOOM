module FieldStatTool

    missing_value = 1e20
# =====  [BEGIN] StatObj Section ===== #
    ...
    mutable struct Recorder
        ...
        function Recorder(dims, vars)
            ...
            for (varname, varref, dimnames) in vars
                ...
                sobjs[varname] = StatObj(varname, varref, dims_name)
            end
    ...
    function record!(
        ...
        if avg_and_output
            ...
            for varname in varnames
                sobj = rec.sobjs[varname]
                if sobj.weight == 0
                    ErrorException(format("StatObj for variable `{:s}` has weight 0 during normalization.", sobj.varname)) |> throw
                end
