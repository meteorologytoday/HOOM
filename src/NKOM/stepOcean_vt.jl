...
function stepOcean_vt!(
    ...
    for substep = 1:substeps
        ...
        for grid_idx in 1:size(ocn.valid_idx)[2]
            ...
            new_S_ML = max( (OC_getIntegratedSalinity(   ocn, i, j; target_z = -new_h_ML) - surf_Sflx * dt)    / new_h_ML, 0.0)
            new_T_ML =      (OC_getIntegratedTemperature(ocn, i, j; target_z = -new_h_ML) - surf_Tnswflx * dt) / new_h_ML
            ...
            if rad_scheme == :exponential
                FLDO = ocn.FLDO[i, j]
                ocn.T_ML[i, j] += - ocn.R * surf_Tswflx * dt / new_h_ML
                ocn.Ts[1:((FLDO == -1) ? Nz : FLDO-1 ), i, j] .= ocn.T_ML[i, j]
                OC_doShortwaveRadiation!(ocn, i, j; Tswflx=(1.0 - ocn.R) * surf_Tswflx, Δt=dt)
            elseif rad_scheme == :step
                ocn.T_ML[i, j] += - surf_Tswflx * dt / new_h_ML
            end
