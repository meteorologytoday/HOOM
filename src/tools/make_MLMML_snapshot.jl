...
Dataset("../../data/domain.ocn.gx3v7.120323.nc", "r") do ds
    global Nx = ds.dim["ni"]
    global Ny = ds.dim["nj"]
    global mask = convert(Array{Float64}, replace(ds["mask"][:], missing=>NaN))
end

Dataset("../../data/clim_LENS_B1850C5CN_005_gx3v7_zMLMML_TEMP.nc", "r") do ds
    global Ts_clim = replace(ds["TEMP"][:, :, :, 1], missing=>NaN)
    global zs = replace(ds["zs"][:], missing=>NaN)
end

Dataset("../../data/clim_LENS_B1850C5CN_005_gx3v7_zMLMML_SALT.nc", "r") do ds
    global Ss_clim = replace(ds["SALT"][:, :, :, 1], missing=>NaN)
end

Dataset("../../data/ocean_topog_gx3v7.nc", "r") do ds
...
MLMML.takeSnapshot(occ, "init_clim_LENS_B1850C5CN_005.nc")
