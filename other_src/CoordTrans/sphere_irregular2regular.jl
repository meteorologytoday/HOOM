...
module WeightGeneration

    struct GridInfo{T} where T <: AbstractFloat
        ...
        function GridInfo{T}(;
            ...
            return new(N, gc_lon, gc_lat, area, mask, unit_of_angle)
        end
    ...
    function genWeight_NearestNeighbors(
        ...
        @sync @distributed for i = 1:gi_s.N
            ...
            s_coord[1, i] = cos(gi_s.lat[i]) * cos(gi_s.lon[i])
            s_coord[2, i] = cos(gi_s.lat[i]) * sin(gi_s.lon[i])
            s_coord[3, i] = sin(gi_s.lat[i])

        end

        @sync @distributed for i = 1:gi_d.N

            d_coord[1, i] = cos(gi_d.lat[i]) * cos(gi_d.lon[i])
            d_coord[2, i] = cos(gi_d.lat[i]) * sin(gi_d.lon[i])
            d_coord[3, i] = sin(gi_d.lat[i])
        ...
        @time @sync @distributed for i = 1:gs_d.N
