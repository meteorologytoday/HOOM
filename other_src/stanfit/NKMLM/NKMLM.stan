...
model{
    ...
    for(m in 1:12) {
        for(k in 1:steps[m]) {
            if (m < 12) {
                heat_cap[step_in_year] = interpolate(k-1.0, 0.0, h[m], steps[m] - 1.0, h[m+1]) * heat_cap_density;
                Q_interp[step_in_year] = interpolate(k-1.0, 0.0, Q[m], steps[m] - 1.0, Q[m+1]);
            } else {
                heat_cap[step_in_year] = interpolate(k-1.0, 0.0, h[12], steps[12] - 1.0, h[1]) * heat_cap_density;
                Q_interp[step_in_year] = interpolate(k-1.0, 0.0, Q[12], steps[12] - 1.0, Q[1]);
            }
    ...
    for(y in 1:years) {
        ...
        for(m in 1:12) {
            ...
            for(k in 1:steps[m]) {
                ...
                predict_T += ( F[t] + Q_interp[step_in_year] - (predict_T - Td) * we[m]) / heat_cap[step_in_year] * sub_dt[m];
