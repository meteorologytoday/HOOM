...
script_path = normpath(joinpath(dirname(@__FILE__), "SOM_fixed_MLD.stan"))
...
Dataset(config["MLD-file"], "r") do ds
    global MLD = convert(Array{Float64}, nomissing(ds["hblt"][target_i, :, 1], NaN))
end
