...
if [ ! -f flag_noplot_mc ]; then
    ...
    python3 $script_plot_dir/plot_mc_timeseries.py --input-dir=$diag_data_dir --output-dir=$graph_dir --res=$res --casenames=$casenames --data-file=atm_analysis2.nc --varname=TREFHT_GLB --ylabel="Temperature [K]" --mavg=12 --extra-title="Yearly average" --colors="$colors"
    python3 $script_plot_dir/plot_mc_timeseries.py --input-dir=$diag_data_dir --output-dir=$graph_dir --res=$res --casenames=$casenames --data-file=atm_analysis2.nc --varname=TREFHT_LND --ylabel="Temperature [K]" --mavg=12 --extra-title="Yearly average" --colors="$colors"
    python3 $script_plot_dir/plot_mc_timeseries.py --input-dir=$diag_data_dir --output-dir=$graph_dir --res=$res --casenames=$casenames --data-file=atm_analysis2.nc --varname=TREFHT_OCN --ylabel="Temperature [K]" --mavg=12 --extra-title="Yearly average" --colors="$colors"
