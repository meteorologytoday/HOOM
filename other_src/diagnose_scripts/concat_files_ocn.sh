...
if [ -f $ocn_concat_rg ] && [ ! -f flag_concat_ocn ]; then
    ...
    if [ -f flag_concat_ocn ] || [ ! -f $ocn_concat ]; then
        ...
        eval "$(cat <<EOF
        ncrcat -h -O -v h_ML,T_ML,dTdt_ent,swflx,nswflx,neb,Q_clim,qflx,qflx2atm,wT,neb,dHdt $full_casename.ocn.h.monthly.{$concat_beg_year..$concat_end_year}.nc $ocn_concat
EOF
        )"
    ...
    cd $wdir
    julia $script_coordtrans_dir/transform_data.jl --s-file=$ocn_concat --d-file=$ocn_concat_rg --w-file=$wgt_file --vars=h_ML,T_ML,dTdt_ent,T_hadvs,T_vadvs,swflx,nswflx,neb,Q_clim,qflx,qflx2atm,wT,neb,dHdt --x-dim=Nx --y-dim=Ny --z-dim=Nz_bone --t-dim=time 
