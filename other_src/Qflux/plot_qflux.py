...
for m in range(len(months)):
    ...
    ax.set_title("%sQ-flux of month %s" % (args.title_prefix, months[m]))

#    print(qflux.shape)
 #   print(lon.shape)
  #  print(lat.shape)

    mappable = ax.contourf(lon, lat, ext(qflux[m, :, :]), clevels, cmap=cmap, extend="both", transform=proj2)
    ...
    cb.ax.set_ylabel('Energy flux [$ \mathrm{W} \, \mathrm{m}^{-2}  $]', rotation=90)

    filename = "%s/%sQflux-%02d.png" % (args.output_dir, args.output_file_prefix, m+1)
