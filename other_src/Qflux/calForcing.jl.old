...
Es_EntSOM15L = zeros(Float64, Nx, Ny, 12)
...
for i=1:Nx, j=1:Ny
    ...
    E[:] /= (nyears - 2)   # remember we discard the first and last year
    E[:] = (E + circshift(E, 1) ) / 2.0

    if any(isnan.(Q)) || any(isnan.(E))
        println(format("Position (i, j) = ({:d}, {:d}) has NaN Q / E flux!", i, j))
    end
...
Dataset(out_file, "c") do ds
    ...
    for o in (
        ...
        [
            ...
            "eflux_EntSOM15L", Es_EntSOM15L, ("Nx", "Ny", "time"), Dict(
            "long_name"=>"E-flux",
            "units"=>"W / m^2",
            )
        ],
